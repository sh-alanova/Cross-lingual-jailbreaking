...
1. Обнаруживать вредоносные запросы без зависимости от языка входа — поддерживая как минимум 4 языка: английский (EN), русский (RU), китайский (ZH), арабский (AR).

2. Работать без машинного перевода на этапе инференса (т.е. не тратить ресурсы на обратный перевод запроса в EN).

3. Использовать фиксированный англоязычный эталонный набор (кодбук) для обнаружения атак на всех языках.

4. Предоставлять бинарное решение: блокировать / пропустить.

Нефункциональные требования:

- Производительность: Время обработки одного запроса ≤ 2 сек (после инициализации)

- Качество: Частота ложных срабатываний (FPR) на безопасных запросах ≤ 30%

- Точность блокировки атак (TPR) ≥ 75%

- Снижение Jailbreak Success Rate (JSR) ≥ 30 п.п. по сравнению с незащищённой моделью

- Надёжность: Стабильность при нагрузке, без сбоев при обработке ≥1000 последовательных запросов
...
- Jailbreak Success Rate (JSR) -- доля вредоносных промптов, на которые LLM дал опасный ответ,
...
- Снижение JSR на ≥10%  по сравнеию с baseline хотя бы на одном неанглийском языке
...
Получены результаты пилота в виде сводной таблицы с JSR, TPR, FPR по всем языкам и бенчмаркам для сценариев с фильтром и без фильтра; проведены эксперименты по подбору порога. 

Таблицы можно найти в папке results/REAMDE.md, а код эксперимента в ноутбуках notebooks/[4] LLMs testing.ipynb и notebooks/[5] ASR Counting.ipynb

Результаты эксперемента по подбору и анализу порога в ноутбуке notebooks/[6] TPR_FPR.ipynb
